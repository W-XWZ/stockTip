...
def index(symbols=None):
    ...
    out = pd.DataFrame(data_list, columns=LIVE_DATA_COLS)
...
def singleStockData(symbol=None):
    symbols_str = ''
    symbols_str+=('sh%s'%symbol if symbol[:1] in ['5', '6', '9'] or symbol[:2] in ['11', '13'] else 'sz%s'%symbol) + ','
    symbols_str = symbols_str[:-1] if len(symbols_str) > 8 else symbols_str
    url="http://hq.sinajs.cn/list=%s"%symbols_str
    response=urllib.request.urlopen(url,timeout=10)
    text = response.read().decode('GBK')
